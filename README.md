...
webview-diff conformance --spec <file>    Figma 디자인 ↔ 구현 일치 검사
...
## 디자인 일치 게이트 (conformance)

위 축들이 "환경마다 같게 그려지나 / 회귀했나"를 본다면, 이건 "구현이 **Figma 디자인대로** 됐나"를 본다.
픽셀 비교가 아니라 구조 비교다: 브라우저에서 `getComputedStyle`/`getBoundingClientRect`로 실제 색·타이포·
간격·반경·크기를 읽어 Figma 노드 토큰과 비교하고, 속성 단위 diff를 낸다. 결정적이고 행동 가능해서
에이전트 루프(수정 → 검사 → 다르면 수정복귀)의 게이트로 쓸 수 있다.
...
근거(픽셀 대신 구조를 쓰는 이유), 에이전트 루프 수렴 가드, Figma 연결, 한계는 [DESIGN-CONFORMANCE.md](./DESIGN-CONFORMANCE.md) 참고.
