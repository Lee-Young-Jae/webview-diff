...
- 성능만: `CLI perf`
- Figma 디자인 ↔ 구현 일치(구조 비교, 에이전트 게이트용): `CLI conformance --spec <file>` → `.out/conformance.json`의 속성단위 findings로 어디가 디자인과 다른지 짚고 수정 제안. (자세한 근거·루프 가드: DESIGN-CONFORMANCE.md)
